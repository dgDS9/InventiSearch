...
def extract_product_lines(text: str) -> list[dict]:
    ...
    stop_pattern = re.compile(r"(subtotal|iva|total de la oferta|condiciones|quedamos)", re.IGNORECASE)
    ...
    for i, line in enumerate(lines):
        ...
        # vorherige Zeilen mitnehmen, aber nicht hart auf Position
        for j in range(max(0, i - 3), i):
            if not stop_pattern.search(lines[j]):
                block_lines.append(lines[j])

        # aktuelle + folgende Zeilen mitnehmen bis Stop-Wort oder nächste SKU
